...
class RetinotopicMappingTrial(object):
        ...
        for key, value in patches.items():
            ...
            if AS / AU >= splitOverlapThr:
                ...
                if NumOfMin == 1:
                    ...
                    if len(newPatches) > 1 and isPlot:

                        f = plt.figure(figsize=(4,1.4))
    ...
    def cleanMaps(self):
        ...
        try:
            del self.altPowerMapf
        except AttributeError:
            pass

        try:
            del self.aziPowerMapf
        except AttributeError:
            pass
    ...
    def plotTrial(self, isSave=False, saveFolder=None):
        ...
        f1_235 = f1.add_subplot(235)
        try:
            currfig = f1_235.imshow(np.mean([self.altPowerMap, self.aziPowerMap], axis=0), cmap='hot',
                                    interpolation='nearest')
            f1.colorbar(currfig)
            plt.axis('off')
        ...
        if isSave:
            f1.savefig(os.path.join(saveFolder, trialName + '_SignMap.pdf'), format='pdf', dpi=600,
                       orientation='landscape', papertype='a4')
            f2.savefig(os.path.join(saveFolder, trialName + '_RawPatches.pdf'), format='pdf', dpi=600,
                       orientation='landscape', papertype='a4')
            f3.savefig(os.path.join(saveFolder, trialName + '_SplitMerge.pdf'), format='pdf', dpi=600,
                       orientation='landscape', papertype='a4')
    ...
    def plotFinalPatchBorders(self, plotAxis=None, plotName=True, plotVasMap=True, isTitle=True, isColor=True,
        ...
        if hasattr(self, 'finalPatchesMarked'):
            ...
            self.processTrial();finalPatches = self.finalPatches
    ...
    def plotPatchesWithName(self, patchDict, plotAxis=None):
        ...
        if not hasattr(self, patchDict):
            raise LookupError('This RetinotopicMappingTrial object does not have "' + patchDict + '" attribute!')
...
def plot_mask_borders(mask, plotAxis=None,
                      color='#ff0000', zoom=1,
                      borderWidth=2, closingIteration=None,
